...
# 配置
EXCEL_FILE = '/mnt/c/Users/44238/Desktop/业务对账数据/4-1/业务对账统计明细-20260402085608 (1).xlsx'
...
def clean_and_import_data():
    ...
    # 只保留4月1日的数据
    df = df[df['业务完成时间'].dt.date == pd.to_datetime('2026-04-01').date()]
    print(f"筛选后4月1日数据: {len(df)} 行")
    ...
    try:
        ...
        print("\n导入数据统计:")
        print(f"  - 总记录数: {len(df)}")
        print(f"  - 有效记录: {inserted}")
        print(f"  - 导入失败: {failed}")
        
        # 显示样例数据
        print("\n样例数据（前3条）:")
        cursor.execute(f"SELECT order_id, institution, amount, order_amount FROM {TABLE_NAME} LIMIT 3")
