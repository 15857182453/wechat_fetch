...
# 海宁四院 - 第三方服务
海宁_four_third = df_filtered[(df_filtered['机构名称'] == '海宁四院') & (df_filtered['业绩类目'] == '第三方服务')]
海宁_four_third_sum = float(海宁_four_third['实际支付金额'].sum()) if len(海宁_four_third) > 0 else 0.0
海宁_four_third_count = int(海宁_four_third['实际支付金额'].count()) if len(海宁_four_third) > 0 else 0

# 海宁四院 - 自营健管
海宁_four_self = df_filtered[(df_filtered['机构名称'] == '海宁四院') & (df_filtered['业绩类目'] == '自营健管')]
海宁_four_self_sum = float(海宁_four_self['实际支付金额'].sum()) if len(海宁_four_self) > 0 else 0.0
海宁_four_self_count = int(海宁_four_self['实际支付金额'].count()) if len(海宁_four_self) > 0 else 0

# 海宁合计
海宁_four_sum = 海宁_four_third_sum + 海宁_four_self_sum
海宁_four_count = 海宁_four_third_count + 海宁_four_self_count
...
第三合计 = (杭州_third_sum + 海宁_four_third_sum + 绍兴_three_sum, 杭州_third_count + 海宁_four_third_count + 绍兴_three_count)
第三方服务 = (第三方[0] - 第三合计[0], 第三方[1] - 第三合计[1])

健管 = from_pivot(pivot, '自营健管')
自营健管 = (健管 [0] - 绍兴_self_sum - 海宁_four_self_sum, 健管 [1] - 绍兴_self_count - 海宁_four_self_count)

心理咨询 = (杭州_third_sum + (海宁_four_third_sum + 海宁_four_self_sum) + 绍兴_three_sum + 绍兴_self_sum,
            杭州_third_count + (海宁_four_third_count + 海宁_four_self_count) + 绍兴_three_count + 绍兴_self_count)
