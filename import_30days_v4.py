...
def main():
    ...
    try:
        ...
        CREATE TABLE daily_flow_details (
            ...
            pay_status TEXT,            -- 列5
            institution TEXT,           -- 列6
            institution_code TEXT,      -- 列7
            province TEXT,              -- 列8
            oper_person TEXT,           -- 列9
            ye_wu_lei_mu TEXT,          -- 列10
            ye_wu_zi_lei_mu TEXT,       -- 列11
            yewu_leixing TEXT,          -- 列12
            yewu_wancheng_shijian TEXT, -- 列20
            caiwu_ruzhang_shijian TEXT, -- 列21
        ...
        new_df['pay_status'] = df.iloc[:, 6]       # ⭐ 收退标识（列5，但索引是6）
        new_df['institution'] = df.iloc[:, 7]      # ⭐ 机构名称（列6，但索引是7）
        new_df['institution_code'] = df.iloc[:, 8] # ⭐ 机构编码（列7，但索引是8）
        new_df['province'] = df.iloc[:, 9]
        new_df['oper_person'] = df.iloc[:, 10]
        new_df['ye_wu_lei_mu'] = df.iloc[:, 11]    # ⭐ 业绩类目
        new_df['ye_wu_zi_lei_mu'] = df.iloc[:, 12] # ⭐ 业绩子类目
        ...
        print("\n前5行关键字段:")
        print(new_df[['pay_status', 'institution', 'ye_wu_lei_mu', 'order_amount']].head())
        ...
        INSERT INTO daily_flow_details (
            ...
            caiwu_ruzhang_shijian, order_amount, amount
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ''', records)
