#!/usr/bin/env python3
"""Replace Tab 11 in dashboard_v4.py"""

with open('/home/openclaw/.openclaw/workspace/dashboard_v4.py', 'r') as f:
    content = f.read()

start_marker = '# ========== TAB 11: 用户行为分析 =========='
end_marker = '# ========== 底部信息 =========='
...
new_tab11 = """# ========== TAB 11: 用户行为分析 ==========
with tab11:
    st.markdown('<div class="card fade-in"><h3>\\U0001f4ca 用户行为分析 \\U000000b7 医院运营月报</h3></div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if not all(k in fx_data for k in ['monthly_4', 'monthly_5', 'rx_4', 'rx_5']):
            ...
            if selected_hospital == "\\u2014 请选择医院 \\u2014":
                st.info("\\U0001f446 请先选择一家医院，查看该院的专属运营数据报告")
                st.markdown(
                    '<div style="text-align:center;padding:50px 20px;color:#94A3B8;">'
                    '<p style="font-size:64px;margin:0;">\\U0001f3e5</p>'
                ...
                st.markdown(
                    ...
                    f'\\U0001f4cb {selected_hospital} \\U000000b7 运营数据报告</div>',
                    unsafe_allow_html=True
                )
                ...
                # 核心指标
                st.subheader("\\U0001f4c8 4月 vs 5月(1-14日) 核心指标对比")

                c1, c2, c3, c4, c5, c6 = st.columns(6)
                c1.metric("\\U0001f4b0 GMV", f"¥{g5:,.0f}", fmt_chg(g5, g4) if r4 else None)
                c2.metric("\\U0001f465 访问人数", f"{v5:,}", fmt_chg(v5, v4) if r4 else None)
                c3.metric("\\U0001f4e6 支付订单", f"{order5:,}", fmt_chg(order5, order4) if r4 else None)
                c4.metric("\\U0001f4b2 客单价", f"¥{avg5:.2f}", fmt_chg(avg5, avg4) if r4 else None)
                c5.metric("\\U0001f504 转化率", f"{conv5*100:.1f}%", fmt_chg(conv5*100, conv4*100) if r4 else None)
                c6.metric("\\U0001f501 60天复购", f"{r60_5*100:.1f}%", fmt_chg(r60_5*100, r60_4*100) if r4 else None)

                st.info("\\U0001f4c5 数据周期：4月(4/1~5/1) vs 5月(5/1~5/14)")
                st.divider()

                # 转化漏斗
                st.subheader("\\U0001f53b 用户转化漏斗 (5月)")
                ...
                # 药方维度
                st.subheader("\\U0001f48a 药方维度数据 (5月)")
                rx5 = fx_data['rx_5']
                rx5_rows = [{
                    '药方名称': r[1],
                    '是否需问卷': r[5],
...
"""
    ...
    new_content = content[:start_idx] + new_tab11 + content[end_idx:]

    with open('/home/openclaw/.openclaw/workspace/dashboard_v4.py', 'w') as f:
        f.write(new_content)

    print(f"✅ Replaced Tab 11 ({len(old_section)} chars -> {len(new_tab11)} chars)")
