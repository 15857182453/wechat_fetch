#!/usr/bin/env python3
"""导入 4 月 13 日明细数据（业务日期 4/11-4/13）到 daily_flow_2026_apr"""
import pandas as pd
import sqlite3
...
# 找到最新文件
files = [f for f in os.listdir('/home/openclaw/.openclaw/workspace') if '20260413071236' in f]
EXCEL = f'/home/openclaw/.openclaw/workspace/{files[0]}'
print(f'📁 使用文件：{files[0]}')
...
df.columns = [
    ...
    '商品子类别','运营分类','商品 id','商品名称','业务完成状态','业务完成时间',
...
# 显示业务日期分布
df['业务完成时间'] = pd.to_datetime(df['业务完成时间'])
print(f'📅 业务日期范围：{df["业务完成时间"].min()} 到 {df["业务完成时间"].max()}')
print(f'📅 唯一日期数：{df["业务完成时间"].dt.date.nunique()}')
...
for _, row in df.iterrows():
    records.append({
        ...
        'yewu_wancheng_shijian': str(row['业务完成时间']) if pd.isna(row['业务完成时间']) else (row['业务完成时间'].strftime('%Y-%m-%d %H:%M:%S') if hasattr(row['业务完成时间'], 'strftime') else str(row['业务完成时间'])),
